...
Reasoning behind the open items in `IDEA.md`. That file is parsed by IdeaBRD
and has to stay a short intro plus a flat `## Todos` list, so the detail lives
here.

## The API is unauthenticated and appears to be internet-facing
...
**Confirm the exposure first** — curl the FQDN from off-network. If it answers,
pick one: a Traefik `forwardAuth`/basic-auth middleware, an OIDC proxy, or
restricting the ingress to the LAN and reaching it over Tailscale. `main.py` is
already structured so middleware can be added without touching routers.

The webhook endpoint has to stay publicly reachable for Plaid to deliver
`SYNC_UPDATES_AVAILABLE`, so it needs signature verification rather than
network restriction.

## Plaid webhook JWT verification is not implemented
...
## `sync_all` cannot actually survive a failing Item
...
## Category filtering ignores overrides
...
## The rules engine has no way to create rules
...
Rules are applied **only at sync time**, so adding a rule doesn't recategorize
existing rows. Wants a `python -m app.cli recategorize` backfill alongside
whatever creates rules.

## CNPG backups are configured but never taken
...
and `build-deploy.yml` goes straight to building images. Highest-value targets,
roughly in order:
...
- **`routers/transactions.py` imports `_month_bounds`** from `services.sankey`
  — a private name across module boundaries. Promote it.
...
- Overrides live in their own table rather than a column, so a sync can refresh
  every synced field without clobbering manual corrections.
- The API serves routes at root; `/api` is stripped by the Traefik `strip-api`
  middleware in-cluster and by the Vite proxy locally. Changing one means
  changing the other.
- CI commits image-tag bumps back to `main` with `[skip ci]`, and
  `paths-ignore` keeps chart-only pushes from rebuilding images.
- `fernet_key` encrypts Plaid access tokens in Postgres. Rotating it orphans
  every linked Item and forces a re-link through Plaid Link.
